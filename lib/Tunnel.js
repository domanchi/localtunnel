...
module.exports = class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    const { id, ip, port, url, cached_url, max_conn_count, apikey } = body;
  ...
  _init(cb) {
    ...
    //  TODO(domanchi): It would be nice if this was configurable.
    //  Also, adding increasing back-off would also be nice.
    let numRetriesLeft = 3;
    const getUrl = () => {
      axios
        .get(uri, params)
        .then(res => {
        ...
        .catch(err => {
          if (
            ...
            [400, 403, 404].indexOf(err.response.status) > -1
          ) {
            return cb(new Error(JSON.stringify(err.response.data)));
          }

          if (--numRetriesLeft <= 0) {
            return cb(new Error("Unable to connect to tunnel server."));
          }
